...
class ImgLoader(object):
    ...
    def load(self, image_path: str):
        ...
        if ori_img is None:
            raise ValueError(f"Image at path '{image_path}' could not be loaded. Please check the path and the file format.")
        
        if ori_img.shape[2] != 3:
            raise ValueError("3 (RGB) channels are required.")
    ...
    def load_from_array(self, image_array):
        ori_img = copy.deepcopy(image_array)
        print(ori_img)
        assert ori_img.shape[2] == 3, "3 (RGB) channels are required."
        ori_img = ori_img[:, :, ::-1]  # Convert BGR to RGB
